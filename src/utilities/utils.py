...
def readJson(str_path: str) -> tuple[dict, str]:
        """Read JSON file"""
        ...
        if json_document.isNull():
            err = f"Parser Error: {json_error.errorString()}"
            return document, err
        
        document: dict = json_document.object()
        for key in document.keys():
            item: QtCore.QJsonValue = document.get(key)
            if item.isArray():
                document[key] = item.toArray()
