...
class ListView(QtWidgets.QListView):
    def __init__(self):
        ...
        self._model = ItemModel(0, 1, self)
    ...
    def insert(self, thing, row):
        item = self.build_item(thing)
        self.selected = row
        self._model.insertItem(item, row)
        self.setCurrentIndex(self._model.index(row))

    def items_deleted(self):
        self.setCurrentIndex(self._model.index(self.selected))

    def remove(self, row):
        self._model.removeItem(row)
    ...
    def update(self, thing):
        item = self.build_item(thing)
        self._model.updateItem(self.currentIndex(), item)
    ...
    def paint(self, painter, option, index):
        ...
        if title:
            ...
            if pinned:
                ...
                title = "     " + title
            painter.drawText(rectangle, QtCore.Qt.AlignmentFlag.AlignVCenter, title)
        if line:
            point = option.rect.topLeft()
            ...
            painter.setFont(font)
            painter.drawText(rectangle, QtCore.Qt.AlignmentFlag.AlignVCenter, line)
        painter.restore()


class ItemModel(QtCore.QAbstractListModel):
    TitleRole, LineRole, PinRole = range(QtCore.Qt.ItemDataRole.UserRole, QtCore.Qt.ItemDataRole.UserRole + 3)
    ...
    def setData(self, index, item, role=QtCore.Qt.ItemDataRole.EditRole):
...
class View:
    def __init__(self):
        ...
        # self.app.setAttribute(QtCore.Qt.ApplicationAttribute.AA_UseHighDpiPixmaps)
        self.main_window = MainWindow()
        self.text_edit = TextEdit()
        self.dock = QtWidgets.QDockWidget()
        self.list_view = ListView()
        self.dock.setWidget(self.list_view)
        self.dock.setFeatures(self.dock.DockWidgetFeature.NoDockWidgetFeatures)
        self.dock.setTitleBarWidget(QtWidgets.QWidget())
        self.main_window.setCentralWidget(self.text_edit)
        self.main_window.addDockWidget(QtCore.Qt.DockWidgetArea.LeftDockWidgetArea, self.dock)
